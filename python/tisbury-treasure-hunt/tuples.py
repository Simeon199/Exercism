...
def compare_records(azara_record, rui_record):
    ...
    azara_record_result = ""
    rui_record_result = ""
    for index, item in enumerate(azara_record[1]):
        azara_record_result += item
        rui_record_result += rui_record[1][index]
    return azara_record_result == rui_record_result
...
def clean_up(combined_record_group):
    ...
    for item in combined_record_group:
        new_inner_tuple = tuple([item[0]]) + item[2:]
        result += str(new_inner_tuple) + "\n"
    return result
